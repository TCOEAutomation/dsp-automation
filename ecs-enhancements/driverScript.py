...
from customUtils import customWriteTestStep, endProcessing, replacePlaceHolders, getIndexNumber, find_element_using_path, customWriteTestCase
    ...
    while SystemConfig.currentRow <= SystemConfig.endRow:
        ...
        if not startingPointisFound:
            if "(start)" not in str(testCaseNumber).lower():
                ...
                SystemConfig.dateFrom=datetime.datetime.now().strftime("%m/%d/%Y %H:%M")
        ...
        if apiToTrigger is None:
            ...
            if typeOfRequest is not None:
                ...
                elif typeOfRequest.lower() == "web":
                    execute_commands_helper.trigger_java_web_validation(requestStructure, apiToTrigger)
                else:
