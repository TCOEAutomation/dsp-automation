...
def validate_user_input_for_excel(val):
    ...
    if api == "cpspromo" or api == "cpsnopromo" or api == "nfnopromo":
        ...
        print "[INF] Running {0} with Arguments: Type={1}, MSISDN={2}, Message={3} Excel={4}" \
              .format(api, val[1], val[2], val[3], val[4])
        command_line = 'mvn test -DAPI="{0}" -DType="{1}" -DMSISDN="{2}" -DMessage="{3}" -DFolder="{4}"' \
              .format(api, val[1], val[2], val[3], val[4])
        print("[DEBUG] Running:: " + command_line)
        return command_line
        ...
        print "[INF] Running {0} with Arguments: Brand={1}, Type={2}, MSISDN={3}, Excel={3}" \
              .format(api, val[1], val[2], val[3])
        command_line = 'mvn test -DAPI="{0}" -DBrand="{1}" -DType="{2}" -DOffer="{3}" -DMSISDN="{4}" -DFolder="{5}"' \
              .format(api, val[1], val[2], val[3], val[4], val[5])
        print("[DEBUG] Running:: " + command_line)
        return command_line
        ...
        if len(val) != expected_length:
            ...
            print_invalid_length(api, len(val), expected_length)
            return -1
        print "[INF] Running {0} with Arguments: MSISDN={1},  DateFrom={2}, Excel={3}" \
              .format(api, val[1], val[2], val[3])
        command_line = 'mvn test -DAPI="{0}" -DMSISDN="{1}" -DDateFrom="{2}" -DFolder="{3}"' \
              .format(api, val[1], val[2], val[3])
        print("[DEBUG] Running:: " + command_line)
        return command_line
...
def trigger_java_web_validation(val, apiToTrigger):
    val = cu.replacePlaceHolders(val)
    ...
    if apiToTrigger.lower() == "databal":
        val = "CURLVerify_DataBal_" + data["MSISDN"] + "_" + data["Message"] + "_" + "DataBal"+timestamp
    elif apiToTrigger.lower() == "groupstatus":
        val = "CURLVerify_GroupStatus_"+data["MSISDN"]+"_" + data["Message"] + "_"+"GroupStatus"+timestamp
        ...
        val = "NFPromo_"+data["MSISDN"]+"_"+data["Status"]+"_" + data["Type"] + "_" + data["Value"] + "_"+"NFPromo"+timestamp
    elif apiToTrigger.lower() == "addmember":
        val = "CURLProvision_addmember_addmember_"+data["Member"] + "_" +data["Owner"]+"_"+"Addmember"+timestamp
    elif apiToTrigger.lower() == "removemember":
        val = "CURLProvision_removemember_removemember_"+data["Member"]+ "_" +data["Owner"]+"_"+"Removemember"+timestamp
    elif apiToTrigger.lower() == "cpsnopromo":
        val = "CPSNoPromo_"+data["MSISDN"]+"_"+data["Status"]+"_"+"CPSNoPromo"+timestamp
...
def parse(vars):
    ...
    for val in allVars:
        ...
        if val.lower().startswith("sleep"):
            ...
            (initialBalance, consumed_data, expectedValue) = val.split("_")
            consumed_data = consumed_data.replace("mb","")
            consumed_data = float(consumed_data) * 1024
            arithmeticExpression = initialBalance + "-" + str(consumed_data)
            keywordHandling.validate_math(arithmeticExpression, expectedValue)
